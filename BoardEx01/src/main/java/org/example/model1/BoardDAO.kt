...
class BoardDAO {
    ...
    init {
        try {
            ...
            this.dataSource = envCtx.lookup("jdbc/mariadb1") as DataSource
    ...
    fun boardWriteOk(to: BoardTO): Int {
        ...
        try {
            ...
            val sql = "insert into board1 values ( 0, ?, ?, ?, password( ? ), ?, 0, ?, now() )"
            pstmt = conn.prepareStatement(sql)
    ...
    fun boardModifyOk(to: BoardTO): Int {
        ...
        try {
            ...
            // 비밀번호는 select 하지 않음
            val sql = "update board1 set subject=?, mail=?, content=? where seq=? and password=password(?)"
            pstmt = conn.prepareStatement(sql)
            pstmt.setString(1, to.subject)
